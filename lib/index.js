...
import { createUserMessage, expandAssistantStream, joinAssistantStreamText } from "@deepseek-ai/dsh-llm";
...
* @param stream - 一次 durable Assistant 结算的压缩流记录。
*/
function foldAssistantStream(stream) {
	let text = "";
	let reasoning = "";
	for (const delta of assistantDeltas(stream)) if (delta.kind === "text") text += delta.text;
...
function applyTranscriptEvent(view, event) {
	...
	switch (event.type) {
		...
		case "assistant/attempt": {
			...
			const prior = base.streaming !== void 0 && base.streaming.turn === turn && base.streaming.step === step ? base.streaming : void 0;
